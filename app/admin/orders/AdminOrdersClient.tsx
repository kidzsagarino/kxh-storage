...
function buildWeekOptions(weeksBack = 10) {
  const now = new Date();
...
function buildMonthOptions(monthsBack = 18) {
  const now = new Date();
  const out = [];
...
function buildYearOptions(yearsBack = 6) {
  const now = new Date();
  const out = [];
              ...
              <tbody className="divide-y divide-slate-100">
                {orders.map((o) => (
                  <tr key={o.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-4">{o.notes}</td>

                    <td className="px-4 py-4">
                      <div className="font-medium text-slate-900">
                        {o.serviceDate
                          ? new Date(o.serviceDate).toLocaleDateString("en-GB", {
                              day: "numeric",
                              month: "short",
                            })
                          : "—"}
                      ...
                      <div className="text-[10px] text-slate-500 uppercase">
                        {o.timeSlot
                          ? `${to12Hour(o.timeSlot.startTime)} - ${to12Hour(o.timeSlot.endTime)}`
                          : "—"}
                      </div>
                    </td>
