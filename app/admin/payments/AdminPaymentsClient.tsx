...
export default function AdminPaymentsClient() {
  ...
  const stats = React.useMemo(() => {
    ...
    const paid = baseList.filter((p) => p.status === "SUCCEEDED").reduce((a, b) => a + b.amount, 0);
    const unpaid = baseList.filter((p) => p.status === "PROCESSING").reduce((a, b) => a + b.amount, 0);
    const refunded = baseList.filter((p) => p.status === "REFUNDED").reduce((a, b) => a + b.amount, 0);

    return {
      paidCount: baseList.filter((p) => p.status === "SUCCEEDED").length,
      unpaidCount: baseList.filter((p) => p.status === "PROCESSING").length,
